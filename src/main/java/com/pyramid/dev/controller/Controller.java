...
//				String eve = Utile.convertJsonToString(ob);
	...
	public Response getShift(@PathVariable("id") Long id, @PathVariable("debut") String debut, @PathVariable("fin") String fin,  Caissier  caissier) {
		...
		int invk = 0;
		int invkOut = 0;
		double balanceInit = 0;
		Airtime airtime;
		caissier.setIdCaissier(id);
		ShiftDay shiftday = new ShiftDay();
		try {
			cais = caisservice.findById(caissier);
			sumInk = mskservice.getMiseRK(cais, debut, fin);
			invk = mskservice.getIntvTicketK(cais, debut, fin);
			sumOutK = verservice.getVersementD(debut, cais, fin,Jeu.K.getValue());
			invkOut = verservice.getPayTicket(cais, debut, fin, Jeu.K.getValue());

			airtime = airtservice.find(cais);
			balanceInit = airtime.getBalance();

			shiftday.setCashink(sumInk);
			shiftday.setCashoutk(sumOutK);
			shiftday.setSlipk(invk);
			shiftday.setVslipk(invkOut);
			shiftday.setBalancein(balanceInit);
	...
	public Response saveUser(@RequestBody CaissierDto cais, @PathVariable("coderace") String coderace) {
		...
		if(p != null) {
			...
			if(caisse == null) {
				...
				try(Response res = caisservice.create(c);) {
				...
				log.info("Caissier deja existant.");
...
//		c.setProfil(prof);
	...
	public Response retrieveCombi(@PathVariable("coderace") String coderace, @PathVariable("num") int num) {
		...
		double gmp;
		double bonusrate;
		double miseTotale_s = 0;
		double percent;
		double lastMiseTotale;
		double lastGainTotal;
		Map<Miset, Misek> mapTicket = new HashMap<>();
		int refill;
		int xtour;
		int position;
		int tour;
		...
		try {
			...
			if(!listTicket.isEmpty()) {
				...
				gMp = supermanager.verifTicketMax(mapTicket);
				gmp = supermanager.verifTicketMin(mapTicket);
				...
				double rounded = supermanager.getRoundPayed(percent, roundSize);
	...
	public Response getMiset(@PathVariable("coderace") String coderace, @PathVariable("t1") Long t1, @PathVariable("t2") Long t2) {
		...
		try {
			...
			p = partnerservice.findById(coderace);
			if (p == null ) return Response.ok(ResponseData.getInstance().error("")).build();
			// log.info("Partner: {}", p.getCoderace());
...
//				 c.setMdpc("***");
	...
	public Response getBonus(@PathVariable("coderace") String coderace) {
		...
		try {
			...
			lknr = lm.stream().map(this::computeKeno).collect(Collectors.toList());
			ob.put("bonus", lknr);
			String eve = Utile.convertJsonToString(ob);
			return Response.ok(ResponseData.getInstance().event(eve).sucess("")).build();
		} catch (Exception e) {
	...
	public Response getLdraw(@PathVariable("coderace") String coderace) {
		...
		try {
			...
			lknr = lm.stream().filter(k -> k.getStarted() != 0).map(this::computeKeno).collect(Collectors.toList());
